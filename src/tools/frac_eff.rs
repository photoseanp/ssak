...
/// бумажных бланках сепарационной эффективности фильтров.
#[derive(Clone, Copy, PartialEq, Eq)]
enum YScale {
    Linear,
    Logarithmic,
...
///
/// Границы сравниваются через <= (а не <): значение равно границе
/// должно попадать в более грубый формат (например, 99.0 -> "99%", а не "99.0%"),
/// иначе ключевые точки шкалы не совпадают с ожидаемыми подписями.
...
/// Промежуточные (неподписанные) уровни "недосепарации" (100 - E) между
/// основными подписанными точками 0%/90%/99%/99.9%/99.99% — по одному набору
/// мелких делений на каждый "лог-декадный" интервал шкалы, как на бумажных
/// бланках сепарационной эффективности: между 0% и 90% — через 10%, между
/// 90% и 99% — через 1%, между 99% и 99.9% — через 0.1%, между 99.9% и 99.99% —
/// через 0.01%.
fn minor_grid_shortfalls() -> Vec<f64> {
...
/// линии. Между этими подписанными уровнями дополнительно нанесены
/// неподписанные промежуточные линии сетки (см. `minor_grid_shortfalls`),
/// как на бумажном бланке-образце. Точка E >= 100% на графике невозможна
/// физически и обрезается по минимально допустимому "недосепарации" 0.001%
/// (т. е. 99.999%), чтобы избежать log(0).
fn plot_data_log(
    series: &[(String, Vec<f64>, Vec<f64>)],
    output_path: &Path,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Промежуточные (неподписанные) линии сетки между основными уровнями —
    // рисуются поверх мелкой сетки конфигуратора, но под данными серий,
    // чтобы визуально не мешать графикам и легенде.
    let minor_style = ShapeStyle {
        color: BLACK.mix(0.15),
        filled: false,
        stroke_width: 1,
    };
    for shortfall in minor_grid_shortfalls() {
        chart.draw_series(std::iter::once(PathElement::new(
            vec![(x_min, shortfall), (x_max, shortfall)],
            minor_style,
        )))?;
    }
...
mod tests {
    ...
    #[test]
    fn minor_grid_shortfalls_are_between_major_levels() {
        let vals = minor_grid_shortfalls();
        assert!(!vals.is_empty());
        for v in &vals {
            assert!(*v > 0.001 && *v < 100.0);
        }
        // Пример минорной линии в первом декадном интервале (0-90%): 50%.
        assert!(vals.iter().any(|v| (*v - 50.0).abs() < 1e-9));
        // Пример минорной линии во втором интервале (90-99%): 95%.
        assert!(vals.iter().any(|v| (*v - 5.0).abs() < 1e-9));
    }
}
